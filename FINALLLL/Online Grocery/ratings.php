    ...
    <div class="top-brands">
        <div class="container">
            ...
            <div class="agile_top_brands_grids">
                <?php
                error_reporting(0);
                $orderData = "SELECT DISTINCT pid FROM order_items WHERE uid = {$_SESSION['USER_ID']}";
                $orderResult = $conn->query($orderData);
                if ($orderResult->num_rows) {
                    while ($orderProduct = $orderResult->fetch_assoc()) {
                
                $result = $conn->query('SELECT * FROM `product` WHERE pid = ' . $orderProduct['pid']);
                if ($result->num_rows) {
                                        ...
                                        <figure>
                                            <div class="snipcart-item block">
                                                <div class="snipcart-thumb">
                                                    ...
                                                    <h4>
                                                        Rs.</i> <?php echo $new_price; ?>
                                                        <span>
                                                            <?php if ($discount) { ?>
                                                                Rs.</i> <?php echo $product['price'];
                                                                    } ?>
                                                        </span>
                                                    </h4>
                                                </div>
                                                <div class="snipcart-details top_brand_home_details">
                                                    <form action="checkout.php" method="post">
                                                        <fieldset id="product">
                                                            ...
                                                            $findingQuery = "SELECT * FROM ratings WHERE uid = {$user_id} AND pid = {$pid}";

                                                            $findingResult = $conn->query($findingQuery);
                                                            $data = mysqli_fetch_assoc($findingResult);
...
<script>
    let sendAjax = (product_id, user_id, total_rating) => {
        $.ajax({
            type: "POST",
            url: "/Online Grocery/action_rating.php",
            data: {"user_id" : user_id, "product_id": product_id, "total_rating": total_rating},
            success: function(result) {
                console.log(result)
            }
        });
    }
    let changeStar = (stars, test) => {
        stars.forEach((f, i) => {
            if (i <= test) {
    ...
    products.forEach(e => {
        let stars = e.querySelectorAll("span")
        let test = 2
        stars.forEach((f, i) => {
            f.addEventListener('click', () => {
                changeStar(stars, i)
                let product_id = f.parentElement.querySelector("input[name='pid']").value
                sendAjax(product_id, user_id, i + 1)
            })
        })
    })
</script>
